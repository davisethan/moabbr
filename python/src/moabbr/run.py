...
def nma(results: pd.DataFrame, greater_is_better: bool = True) -> dict:
    ...
    Returns:
        ...
        - **p_scores** (`dict[str, float]`): P-score per pipeline. Higher is
          better. Frequentist analogue of SUCRA.
        - **league** (`dict`): Pairwise league table with keys `md`, `lower`,
          `upper` (95% CI), `z`, and `pval` (two-sided).
        - **heterogeneity** (`dict`): `tau2`, `tau` (DL point estimates);
          `i2`, `i2_lower`, `i2_upper` (95% CI); `q`, `q_df`, `q_pval`.
        - **prediction** (`dict`): Prediction interval matrix (`lower`,
          `upper`) on the outcome scale — expected range of true effects in a
          new dataset.
    """
...
def bnma(results: pd.DataFrame, greater_is_better: bool = True) -> dict:
    ...
    Returns:
        ...
        - **sucra** (`dict[str, float]`): SUCRA per pipeline. Higher is
          better. Bayesian analogue of P-scores.
        - **league** (`dict`): Pairwise league table with keys `md` (posterior
          median), `lower`, `upper` (95% credible intervals). There is a 95%
          posterior probability the true effect lies within this range.
        - **heterogeneity** (`dict`): `sd` (posterior median of between-study
          SD, Bayesian analogue of tau), `sd_lower`, `sd_upper` (95% credible
          interval).
        - **convergence** (`dict`): `rhat_max` (< 1.01), `ess_bulk_min`
          (> 400), `ess_tail_min` (> 400).
    """
